...
class MovieLib:
    ...
    def add(self, title, date, runtime):
        ...
        node = Movie(title, date, runtime)
        # you need to create the Movie object, then add it to the BST,
        if self.bst is not None:
            return self.bst.add(node)
        else:
            self.bst = BSTNode(node)
            return node
...
# MovieLib._test()
library_set = []
small_library = build_library('smallmovies.txt')
repeated_library = build_library('small_repeated_movies.txt')
Largelibrary = build_library('movies.txt')
library_set.append(small_library)
library_set.append(repeated_library)
library_set.append(Largelibrary)
for m in library_set:
    node = m.search("Wonder Woman")
    if node is not None:
        print(node.full_str())
    node = m.search("Touch of Evil")
    if node is not None:
        print(node.full_str())
    node = m.search("Delicatessen")
    if node is not None:
        print(node.full_str())
